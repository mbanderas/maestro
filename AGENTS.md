...
Never spawn more than 4 specialist sub-agents per parallel group. Three
...
The Planner is always the first sub-agent spawned in multi-agent mode.
...
The Staff Engineer is the final sub-agent spawned. It reviews the
integrated output of all specialists before delivery.
...
These rules govern YOUR behavior as the main agent operating in
multi-agent mode.
...
Compaction silently destroys context and you will edit against stale
state.

**Verify before trusting reads.** For large files, do not assume a
single read captured everything. If the file is over 500 lines, read
in sections and confirm you have the complete picture before editing.

**Tool result truncation.** Search and command outputs may be silently
truncated. If results seem suspiciously sparse, re-run with narrower
scope (single directory, stricter glob). State explicitly when you
...
- Run the project's type-checker (e.g., `npx tsc --noEmit` or equivalent)
- Run the project's linter (e.g., `npx eslint . --quiet` or equivalent)
- Run the project's test suite if specified in this file or in project docs
...
to confirm the change applied correctly. Edits can fail silently when
the target content does not match due to stale context.
...
or modifying any function, type, variable, or constant, search
separately for:
...
Do not assume a single search captured everything. Assume it missed
...
**Senior dev standard.** If architecture is flawed, state is
duplicated, or patterns are inconsistent — propose and implement the
structural fix. Ask yourself: "What would a senior engineer reject
in code review?" Fix all of it.
