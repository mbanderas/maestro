...
- **Settings**: a unified `/maestro:settings` picker (keyboard-driven, like `/model`) plus a portable `node settings/cli.cjs status|set` for Codex and any other CLI, over the terse, frontier, and context-bar toggles. [`docs/settings.md`](docs/settings.md)
